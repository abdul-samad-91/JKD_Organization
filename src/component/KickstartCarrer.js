  ...
  return (
    <section className='bg-[#005f88] w-full flex flex-col items-center my-10 py-20 text-white'>
        ...
        <div  className='max-w-[1200px] px-4 pt-10 '>
          <div>
              <h2 className='text-[35px] font-semibold text-center '>Why Choose <span className={` ${theme === 'dark' ? 'text-[#177faa]' : 'text-[#e98b28]'}`}>Us?</span></h2>
          </div>
          <div className='md:flex  gap-5 rounded justify-center items-center md:pt-5'>
                {data?.map((card , index)=>
                  ...
                  >
                    <h3 className='text-[30px] font-semibold w-[60%]  text-black'>{card.title} <span className={` ${theme === 'dark' ? 'text-[#177faa]' : 'text-[#00874F]'}`}>{card.titlePart}</span></h3>
